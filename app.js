const {Lab, Atc_code,Icd10_code,
    class_drug, 
    class_measurement, 
    class_condition, class_visit,class_person,class_distribution_result,class_protocol_condition,class_protocol_measurement,class_distribution_result_measurement } = require('./cdm/class.js');
...
const { cdm } = require('./cdm/cdm_table'); 
const site_execute = require('./cdm/site_execute.js');



//SITE
app.get('/site-execute/:study_id/:query_id',(req,res)=>{
    ...
    async function asyncMain(study_id, query_id) {
           ...
           // console.log({rows_us:rows_us});
            ...
            try{
                ...
                for(i=0;i<in_ex.ex.length;i++){
                    ...
                    ud_str_or_weak  = asyncMain_2_0_assort_strength(ud_result); 
                    // console.log({result_str_or_weak: str_or_weak_or_ud}); // 강한조건, 유연조건 나누기, User_define
                    ud_query_list = asyncMain_2_1_make_query(ud_str_or_weak,1);
                    //console.log({ud_query_list:ud_query_list})
                    ud_query = `SELECT distinct ud.person_id FROM (${ud_query_list[0]}) ud`
        ...
        function make_query_1(inclusion, exclusion, visit){
            ...
            if (inclusion.length == 0 || exclusion == 0) {
                query = "SELECT DISTINCT provider_id, A.person_id, A.year_of_birth, A.gender_concept_id FROM ( " + inclusion + " ) A  "
            } else {
                query = "SELECT DISTINCT provider_id, A.person_id, A.year_of_birth, A.gender_concept_id FROM ( " + inclusion + " ) A LEFT JOIN ( " + exclusion + " ) B ON A.person_id = B.person_id WHERE B.person_id IS NULL"
            }
    ...
    async function asyncMain_protocol(study_id, query_id) {
        ...
        try{
           ...
           //console.log(study_id, query_id);
            ...
            ////Condition
            q_con = 'SELECT  f1.person_id, f1.gender_concept_id, f1.condition_concept_id, f1.condition_start_date, c1.concept_name, i1.mid as icd10_mid, i1.small as icd10_small, i1.CONCEPT_NAME as icd10_concept_name  FROM (SELECT d1.person_id, p1.gender_concept_id, d1.condition_concept_id, d1.condition_start_date FROM (SELECT person_id, condition_concept_id, condition_start_date FROM dbo.CONDITION_OCCURRENCE WHERE dbo.CONDITION_OCCURRENCE.person_id in('+q_protocol_query_3+')) d1, dbo.PERSON p1 WHERE d1.person_id = p1.person_id and d1.condition_start_date >= '+ date_start+') f1, concept c1 , mtdb.dbo.icd10_snomed_category i1 WHERE f1.condition_concept_id = c1.concept_id AND f1.condition_concept_id = i1.concept_id';
            ...
            ////Visit
            visit_q = 'SELECT p1.person_id, p1.gender_concept_id, d1.visit_concept_id, d1.visit_start_date, d1.visit_end_date, d1.care_site_id, d1.visit_occurrence_id FROM (SELECT person_id, visit_concept_id, visit_start_date, visit_end_date,care_site_id, visit_occurrence_id FROM dbo.VISIT_OCCURRENCE WHERE dbo.VISIT_OCCURRENCE.person_id in( '+q_protocol_query_3+' )) d1, dbo.PERSON p1 WHERE d1.person_id = p1.person_id and d1.visit_start_date >= '+date_start 
