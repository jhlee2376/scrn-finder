    ...
    async function asyncMain(study_id, query_id) {
        ...
        function asyncMain_2_1_make_query(rows_str_weak, condition ){
            ...
            if(rows_str_weak.visit!=null && rows_str_weak.visit.length!=0){
                ...
                visit = `SELECT person_id FROM VISIT_OCCURRENCE WHERE person_id >0 ${val} `
            }
        ...
        function make_query_1(inclusion, exclusion, visit){
            ...
            // visit 적용
            if(visit.length!=0){
                query = `SELECT F.provider_id, F.person_id, F.year_of_birth, F.gender_concept_id FROM ( ${query} ) F WHERE F.person_id in ( ${visit} )`;
            }
        ...
        try{
           ...
           let result = await pool.request() 
            ...
            let result_measurement = await pool.request()
                .query('SELECT f1.gender_concept_id, f1.measurement_concept_id, f1.measurement_date, f1.value_as_number, f1.range_low, f1.range_high, f1.unit_source_value, c1.concept_name FROM (SELECT p1.gender_concept_id, d1.measurement_concept_id, d1.measurement_date, d1.value_as_number, d1.range_low, d1.range_high, d1.unit_source_value FROM (SELECT person_id, measurement_concept_id, measurement_date, value_as_number, range_low, range_high, unit_source_value FROM dbo.MEASUREMENT WHERE dbo.MEASUREMENT.person_id in('+person_list+')) d1, dbo.PERSON p1 WHERE d1.person_id = p1.person_id and d1.measurement_date >='+ date_start +') f1, concept c1 WHERE f1.measurement_concept_id = c1.concept_id');
            ...
            let result_condition = await pool.request()
                .query('SELECT  f1.gender_concept_id, f1.condition_concept_id, f1.condition_start_date, c1.concept_name  FROM (SELECT p1.gender_concept_id, d1.condition_concept_id, d1.condition_start_date FROM (SELECT person_id, condition_concept_id, condition_start_date FROM dbo.CONDITION_OCCURRENCE WHERE dbo.CONDITION_OCCURRENCE.person_id in('+person_list+')) d1, dbo.PERSON p1 WHERE d1.person_id = p1.person_id and d1.condition_start_date >= '+ date_start+') f1, concept c1 WHERE f1.condition_concept_id = c1.concept_id');
            ...
            console.log(distribution_condition[0])
            ...
            console.log(distribution_condition.length);
            console.log({distribution_condition:distribution_condition});





           // console.log({distribution_condition:result1.condition_info});
    ...
    async function asyncMain(study_id, query_id) {
        try{
           ...
           //console.log(study_id, query_id);
            ...
            .input('input_paramiter1', sql.Int, study_id)
            .input('input_paramiter2', sql.Int, query_id)
            .query(`UPDATE patient_query SET query_status = N'Done' WHERE study_id = @input_paramiter1 and query_id = @input_paramiter2`)
            //console.log(`UPDATE protocol SET protocol_status = N'InProgress' WHERE protocol_id = @input_paramiter`);
...
///위즈모 post버전
app.post('/test-protocol-advanced/:protocol_id',(req,res)=>{
    console.log({req:req.params.protocol_id});
    var protocol_id = req.params.protocol_id;
