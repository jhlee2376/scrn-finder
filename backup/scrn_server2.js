    ...
    async function asyncMain(study_id, query_id) {
        ...
        function asyncMain_2_1_make_query(rows_str_weak, condition ){
            ...
            if(rows_str_weak.visit!=null && rows_str_weak.visit.length!=0){
                ...
                visit = `SELECT person_id FROM VISIT_OCCURRENCE WHERE person_id >0 and ${val} `
            }
        ...
        try{
           ...
           let result = await pool.request() 
            ...
            let result_condition = await pool.request()
                .query('SELECT p1.gender_concept_id, d1.condition_concept_id, d1.condition_start_date FROM (SELECT person_id, condition_concept_id, condition_start_date FROM dbo.CONDITION_OCCURRENCE WHERE dbo.CONDITION_OCCURRENCE.person_id in('+person_list+')) d1, dbo.PERSON p1 WHERE d1.person_id = p1.person_id and d1.condition_start_date >= '+ date_start);
    ...
    async function asyncMain(study_id, query_id) {
        try{
           ...
           //console.log(study_id, query_id);
            ...
            //console.log(`UPDATE protocol SET protocol_status = N'InProgress' WHERE protocol_id = @input_paramiter`);
            var provider_id =result.recordset[0].invest_num;
            ...
            var str_or_weak  = asyncMain_2_0_assort_strength(result);  //console.log({result_str_or_weak: str_or_weak}); // 강한조건, 유연조건 나누기. 
            var query_list = asyncMain_2_1_make_query(str_or_weak, provider_id);             //res.send({resunt:{query_list:query_list}}); sql.close();  
        ...
        function asyncMain_2_1_make_query(rows_str_weak, provider_id){
            util_console_print('------asyncMain_2_1_make_query------'); // console.log(rows_str_weak);
            // 강한 조건에서 다시 조건 하나를 넣어준다. 
            // 조건이 넣어지면 다시 선정 제외조건을 나눠서 쿼리문을 만들어준다. 
            // 강한 조건이 끝나면 다시 약한 조건으로 하나씩 쿼리를 만든다. 
