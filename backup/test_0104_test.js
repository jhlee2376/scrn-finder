...
app.get('/test-site-execute/:study_id/:query_id',(req,res)=>{
    ...
    async function asyncMain(study_id, query_id) {
        ...
        function asyncMain_2_1_make_query(rows_str_weak, condition ){
            ...
            console.log({asyncMain_2_1_make_query_query_list:query_list})
            return query_list;
        ...
        function make_query_0(rows, in_or_ex) {
            ...
            var select_table = ['atc_rxnorm_all', 'icd10_snomed_category']; // -5
            ...
            for (i = 0; i < rows.length; i++) {
                ...
                if(rows[i].criteria_addition.length >0 && rows[i].criteria_addition!='null'  && table !=6 && table !=5){
                ...
                switch (Number(table)) {
                    ...
                    case 3:
                        ...
                        if(con ==0){
                            q_measurement = `( SELECT * FROM ${q_measurement_buf} ff1 WHERE 1=1 ${c_add})`
                        }else if(con==1){
                            q_measurement = `( SELECT ff1.person_id, ff1.provider_id, ff1.measurement_date FROM ${q_measurement_buf} ff1 WHERE ff1.measurement_date = ( SELECT MIN(ff2.measurement_date) FROM ${q_measurement_buf} ff2 WHERE ff2.person_id = ff1.person_id) ${c_add} )`
                        }else{
                            q_measurement = `( SELECT ff1.person_id, ff1.provider_id, ff1.measurement_date FROM ${q_measurement_buf} ff1 WHERE ff1.measurement_date = ( SELECT MAX(ff2.measurement_date) FROM ${q_measurement_buf} ff2 WHERE ff2.person_id = ff1.person_id) ${c_add} )`

                        }// con
                        console.log({"검사_테스트":q_measurement})
                        result.measurement.push(q_measurement)
                    ...
                    case 5:
                        ...
                        let q_drug= ''
                        let q_drug_buf = `( SELECT person_id, provider_id, drug_exposure_start_date FROM drug_exposure WHERE drug_concept_id IN (SELECT distinct concept_id FROM mtdb.dbo.atc_rxnorm_all WHERE ${select_lms_atc[field-1]} LIKE \'%${title}%\'))`
                        
                        if(con==0){
                            q_drug = `( SELECT person_id, provider_id FROM drug_exposure WHERE drug_concept_id IN (SELECT distinct concept_id FROM mtdb.dbo.atc_rxnorm_all WHERE ${select_lms_atc[field-1]} LIKE \'%${title}%\') ${c_add})`
                        }else if(con==1){//첫 처방일
                            q_drug = `( SELECT ff1.person_id, ff1.provider_id, ff1.drug_exposure_start_date FROM ${q_drug_buf}  ff1 WHERE ff1.drug_exposure_start_date = (SELECT MIN(drug_exposure_start_date) FROM  ${q_drug_buf} ff2 WHERE ff2.person_id = ff1.person_id ) ${c_add})`
                        }else{
                            q_drug = `( SELECT ff1.person_id, ff1.provider_id, ff1.drug_exposure_start_date FROM  ${q_drug_buf} ff1 WHERE ff1.drug_exposure_start_date = (SELECT MAX(drug_exposure_start_date) FROM  ${q_drug_buf} ff2  WHERE ff2.person_id = ff1.person_id ) ${c_add})`
                        }//if con
                        console.log({"약_테스트":q_drug})

                        result.drug.push(q_drug);
                    ...
                    case 6:
                        ...
                        //console.log(`'${title}'`)
                        let q_condition = '';
                        let q_condition_buf =   `( SELECT person_id, provider_id, condition_start_date FROM  CONDITION_OCCURRENCE WHERE condition_concept_id IN (SELECT distinct concept_id FROM mtdb.dbo.icd10_snomed WHERE ${select_lms[field-1]} LIKE \'%${title}%\') )`  
                        if(con==0){
                            q_condition = `( SELECT person_id, provider_id FROM  CONDITION_OCCURRENCE WHERE condition_concept_id IN (SELECT distinct concept_id FROM mtdb.dbo.icd10_snomed WHERE ${select_lms[field-1]} LIKE \'%${title}%\') ${c_add})`
                        }else if(con==1){
                            q_condition = `( SELECT ff1.person_id, ff1.provider_id, ff1.condition_start_date FROM ( ${q_condition_buf} ) ff1 WHERE ff1.condition_start_date = ( SELECT MIN(condition_start_date) FROM ${q_condition_buf} ff2 WHERE ff2.person_id = ff1.person_id) ${c_add})`
                        }else{
                            q_condition = `( SELECT ff1.person_id, ff1.provider_id, ff1.condition_start_date FROM ( ${q_condition_buf} ) ff1 WHERE ff1.condition_start_date = ( SELECT MAX(condition_start_date) FROM  ${q_condition_buf} ff2 WHERE ff2.person_id = ff1.person_id) ${c_add})`
                        }// if con
                        console.log({"병력_테스트":q_condition})
                        result.condition.push(q_condition);
                        //console.log({c_add:rows[i]})                     
                        break;
                ...
                switch (Number(table)) {
                    ...
                    case 0:
                        if(field==0){ //0 젠더
                            ...
                            result.person.push("( SELECT person_id, provider_id FROM person WHERE 1=1 " +rows[i].criteria_addition+ " )");
                        }
