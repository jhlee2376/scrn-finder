...
app.get('/test-site-execute/:study_id/:query_id',(req,res)=>{
    ...
    async function start(){
        try{
            main_query= await count_start(study_id, query_id,res);
            console.log({main_query:main_query});
            //await asyncMain(study_id, query_id, main_query);
            //await asyncMain_protocol(study_id, query_id, main_query);
            // await asyncMain_patient(study_id, query_id, main_query);
           

            //상태 업데이트 
            let pool_first =  await sql.connect(dbConfig)
            let result_first = await pool_first.request() 
            ...
            util_console_print({"마지막 상태 업데이트": result_first});
            res.send("ok");
        }catch(err){
    ...
    ////Async 함수
    async function asyncMain(study_id, query_id, main_query) {
        try{
           ...
           if(control ==0){
            ...
            query_3_total =` SELECT count(distinct t1.person_id) as cnt FROM (${main_query}) t1`
            pool.close();
            sql.close();
            ...
            query_3 = `SELECT f1.provider_id, f1.person_id, p1.year_of_birth, p1.gender_concept_id FROM (${main_query}) f1, PERSON p1 WHERE f1.person_id = p1.person_id AND f1.provider_id IS NOT NULL`
    ...
    //protocol 함수부분
    async function asyncMain_protocol(study_id, query_id,main_query) {
        ...
        try{
         ...
         if(control==0){
            ...
            query_3 = `SELECT DISTINCT f1.person_id FROM (${main_query}) f1`
            //최종 쿼리 실행 
            console.log({query_3 :query_3});
            result = await pool.request()
            .query(query_3);
            ...
            m_q2 = `SELECT DISTINCT v1.person_id, v1.measurement_concept_id, v1.value_as_number, v1.concept_name FROM ( ${m_q1} ) v1 WHERE v1.measurement_date = ( SELECT MAX(measurement_date) FROM ( ${ m_q1} ) v2 WHERE v2.person_id = v1.person_id)`
            ...
            let result_condition = await pool.request()
                .query('SELECT  f1.person_id, f1.gender_concept_id, f1.condition_concept_id, f1.condition_start_date, c1.concept_name  FROM (SELECT d1.person_id, p1.gender_concept_id, d1.condition_concept_id, d1.condition_start_date FROM (SELECT person_id, condition_concept_id, condition_start_date FROM dbo.CONDITION_OCCURRENCE WHERE dbo.CONDITION_OCCURRENCE.person_id in('+person_list+')) d1, dbo.PERSON p1 WHERE d1.person_id = p1.person_id and d1.condition_start_date >= '+ date_start+') f1, concept c1 WHERE f1.condition_concept_id = c1.concept_id');
            ...
            let distribution_drug           = make_distribution_drug(result1.drug_info); //console.log({distribution_drug:distribution_drug});
        ...
        function make_distribution_condition(condition_info){
            ...
            for(i=0;i<rows.length;i++){        
                //i=0
                if(i==0){
                    condition_list.push(class_protocol_condition(rows[i].condition_concept_id,rows[i].concept_name, rows[i].person_id));
                }else{
                    isNull = list_chk(rows[i].condition_concept_id, condition_list);
            ...
            for(i=0;i<condition_list.length;i++){
                result.push(class_distribution_result(condition_list[i].condition_name, condition_list[i].person_list.length));
        ...
        function make_distribution_drug(drug_info){
            ...
            for(i=0;i<rows.length;i++){        
                //i=0
                if(i==0){
                    condition_list.push(class_protocol_condition(rows[i].drug_concept_id,rows[i].concept_name, rows[i].person_id));
                }else{
                    isNull = list_chk(rows[i].drug_concept_id, condition_list);
                    if(isNull==-1){
                        //console.log('add');
                        condition_list.push(class_protocol_condition(rows[i].drug_concept_id,rows[i].concept_name, rows[i].person_id));
                    }else{
        ...
        function make_distribution_measurement(measurement_info){
            ...
            for(i=0;i<rows.length;i++){        
                //i=0
                if(i==0){
                    measurement_list.push(class_protocol_measurement(rows[i].measurement_concept_id, rows[i].concept_name, rows[i].value_as_number, rows[i].person_id));
                }else{
            ...
            console.log(measurement_list[1]);
            buf_list =[];
            for(i_last=0;i_last<measurement_list.length;i_last++){
                ...
                result.push(class_distribution_result_measurement(measurement_list[i_last].measurement_name, avg, sd, min, max, measurement_list[i_last].person_list.length ));
    ...
    async function count_start(study_id, query_id,res){
        ...
        //console.log(site);
        try{
            out = await site_execute(study_id, query_id, res);
            //res.send('ok');
            return out;
        }catch(err){
    ...
    async function start(study_id,query_id){
        try{
            ...
            if(result !=null){
                ...
                if  (
                    rows[0].measurement_concept_id==4153111    ||
                    rows[0].measurement_concept_id==44816672   || 
                    rows[0].measurement_concept_id==3036671    || 
                    rows[0].measurement_concept_id==3034530    || 
                    rows[0].measurement_concept_id==3004077    
                ){
                  rows[0].concept_name = 'Glucose measurement'  ;
                }else if(
                    rows[0].measurement_concept_id==40758583    ||
                    rows[0].measurement_concept_id==3000963   
                ){
                  rows[0].concept_name = 'Hemoglobin measurement'  ;
                }
                ...
                for(i=1;i<rows.length;i++){
                    ...
                    if (
                            rows[i].measurement_concept_id==4153111    ||
                            rows[i].measurement_concept_id==44816672   || 
                            rows[i].measurement_concept_id==3036671    || 
                            rows[i].measurement_concept_id==3034530    || 
                            rows[i].measurement_concept_id==3004077    
                        ){
                          rows[i].concept_name = 'Glucose measurement'  ;
                        }else if(
                            rows[i].measurement_concept_id==40758583    ||
                            rows[i].measurement_concept_id==3000963   
                        ){
                          rows[i].concept_name = 'Hemoglobin measurement'  ;
                        }
